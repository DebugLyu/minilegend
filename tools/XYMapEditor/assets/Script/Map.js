...
import { runInThisContext, runInDebugContext } from "vm";
...
cc.Class({
    ...
    showGrid() {
        ...
        for (let yTileNum = 0; yTileNum < rowCount; yTileNum++) {
            for (let xTileNum = 0; xTileNum < lineCount; xTileNum++) {
                ...
                this.draw.fillRect(-this.width / 2 + xTileNum * gridWidth, -this.height / 2 + yTileNum * gridHeight, gridWidth - 2, gridHeight - 2);
            }
        }
    ...
    saveBtnClick(e, d) {
        ...
        if (cc.sys.isBrowser) {
            ...
            let textFileAsBlob = new Blob([JSON.stringify(saveInfo)], { type: 'application/json' });
    ...
    checkRolePos() {
        let rolePos = this.map.convertToWorldSpaceAR(this.selfHero.getPosition());
        let distance = cc.v2(this.width / 2, this.height / 2).sub(rolePos).mag();// cc.pDistance(, rolePos);
        if (distance > 1) {
    ...
    updateMap(deltaPos) {
        ...
        let maxX = (this.mapScale - 1) * this.width / 2;
        let maxY = (this.mapScale - 1) * this.height / 2;
        if (this.map.x > maxX) {
            this.map.x = maxX;
        }
        if (this.map.y > maxY) {
        ...
        let minh = (MAP_RES_HEIGHT - this.height) * this.mapScale + maxY;
        let minw = (MAP_RES_WIDTH - this.width) * this.mapScale + maxX;
        if (this.map.y <= -minh) {
            this.map.y = -minh;
        }
        if (this.map.x <= -minw) {
        ...
        console.log('本次寻路耗时1111:', timestamp1 - timestamp, '毫秒');
    },
    ...
    touchBegan(event) {
        ...
        let touchx = Math.floor((drawPos.x + this.width / 2) / gridWidth);
        ...
        if (this.brushType == 9) {
            ...
            let endPosInMap = cc.v2(drawPos.x + this.width / 2 - gridWidth / 2, drawPos.y + this.height / 2 - gridHeight / 2);
        ...
        // this.buildingLayer.removeChildByTag(touchy * lineCount + touchx);
    ...
    getAvailabelPoint(r, l, mapArr, rows, lines, pos) {
        ...
        while (pointarr.length == 0) {
            ...
            if (r + count < rows && l + count < lines && mapArr[r + count][l + count] != 0) {
                pointarr.push({ r: r + count, l: l + count });
            }
        ...
        for (const point of pointarr) {
            let curlength = cc.v2(point.l * gridWidth, point.r * gridHeight).sub(pos).mag();// cc.pDistance(, pos);
    ...
    searchRoad(start_r, start_l, end_r, end_l, mapArr, rows, cols) {
        ...
        openList.push({ r: start_r, l: start_l, G: 0 });//把当前点加入到开启列表中，并且G是0
        ...
        } while (!(result_index = this.existInOpenList({ r: end_r, l: end_l }, openObjList, cols)));
